...
public class UDPMulticastConnector implements Connector {
	 ...
	 */
	public UDPMulticastConnector(InetSocketAddress address) {
		if (address == null) {
	...
	public synchronized void start() throws IOException {
		...
		socket = new MulticastSocket(localAddr.getPort());
		try{
			socket.joinGroup(localAddr.getAddress());
		}catch(NullPointerException e){
			try{
				InetAddress group = Inet6Address.getByName(localAddr.getHostName());
